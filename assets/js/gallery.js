...
(function (document, window, domIsReady, undefined) {
  ...
  domIsReady(function () {
    ...
    function closeLoader(callBack) {
      ...
      aside.style.cssText =
        "background: #fff; left: 50%; position: fixed; top: 50%; opacity: 0; transform: translate(-50%, -50%) scale(0.9); transition: all 0.6s ease-in-out; width: 100%; z-index: 0;";
    ...
    /*------------------------------------
        ...
        --------------------------------------*/
    var lazy = [],
      imageAssetsPath = "/assets/img/";

    function lazyLoad() {
      lazy = document.getElementsByClassName("lazyImg");
      for (var i = 0; i < lazy.length; i++) {
        if (lazy[i].getAttribute("data-src") && isInViewport(lazy[i])) {
          var imagePath = lazy[i].getAttribute("data-src");
          var imageSource = window.origin + imageAssetsPath + imagePath;

          lazy[i].src = imageSource;
    ...
    function cards_navigate() {
      ...
      if (_toSection === cardsCurrentGroup) {
        return;
      }
      document
        .getElementById("nI-" + cardsCurrentGroup)
      ...
      cards_remove_listeners(function () {
        ...
        cards_remove(function () {
          getGalleryThumbnailsConfig(_toSection, function () {
            cards_set_listeners();
            card_show();
          });
        });
    ...
    function cards_set_listeners(callback) {
      ...
      for (var i = 0; i < _cards.length; i++) {
        var card = _cards[i];
        galleryCardInDOMById[cardsCurrentGroup] =
          galleryCardInDOMById[cardsCurrentGroup] || [];
        galleryCardInDOMById[cardsCurrentGroup].push(card.id);
        ...
        if (boundImgListeners.has(card.id)) {
          document
            ...
            .addEventListener("click", boundImgListeners.get(card.id));
          continue;
        }
        boundImgListeners.set(
          card.id,
          showFullGalleryImage.bind(null, {
            gallerySection: cardsCurrentGroup,
            thumbnailID: i + 1,
          })
        );
        document
    ...
    function cards_remove_listeners(callBack) {
      ...
      for (var i = 0; i < _cards.length; i++) {
        ...
        document
          .getElementById(card.id)
          .removeEventListener("click", boundImgListeners.get(card.id));
      }
      return typeof callBack === "function" ? callBack() : null;
    }
    ...
    var galleryThumbnailsConfigLoaded = false,
      thumbnailJSON;
    function getGalleryThumbnailsConfig(gallerySection, callback) {
      var thumbnailsJsonPath =
        window.origin + "/assets/data/nick_101/gallery-thumbnails.json";
      if (galleryThumbnailsConfigLoaded === false) {
        getJson(thumbnailsJsonPath, function (json) {
          thumbnailJSON = json;
          generateAndAppendGalleryThumbnails(
            {
    ...
    function generateAndAppendGalleryThumbnails(params, callback) {
      cards_fragment = cards_fragment || document.createDocumentFragment();
      var sectionConfig = thumbnailJSON[params.gallerySection];

      for (var key in sectionConfig) {
        if (!sectionConfig.hasOwnProperty(key)) continue;

        var imageName = sectionConfig[key];

        var div = document.createElement("div"),
          img = document.createElement("img");

        div.classList.add("card", "hide");
        div.setAttribute("id", params.gallerySection + "-" + key + "-copy");
        div.setAttribute("data-show", params.gallerySection);
        div.innerHTML = '<div class="content"></div>';

        var nick101GalleryThumbnailsPath =
          window.origin + "/assets/img/nick101/gallery-thumbnails";
        var imageFolderPath = params.gallerySection;
        var imageSrcURL =
          nick101GalleryThumbnailsPath +
          "/" +
          imageFolderPath +
          "/" +
          imageName;
    ...
    function showFullGalleryImage(params) {
      ...
      currentImgKey = parseInt(params.thumbnailID);

      // Show galley modal by removing hide class.
      var galleryModal = document.getElementById("aside-modal");
      galleryModal.className = galleryModal.className.replace(" hide", "");

      // Show galley modal loader by removing hide class.
      ...
      if (!galleryLocationsLoaded) {
        ...
        document
          ...
          .addEventListener("click", function () {
            ...
            if (currentImgKey === _size[currentImageSection]) {
    ...
    function updateModalImageSrc(params) {
      ...
      var galleryModal = document.getElementById("aside-modal"),
        galleryModalImageElement = galleryModal.querySelector("#gallery-image");

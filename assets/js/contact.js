...
(function (document, window, domIsReady, undefined) {
  ...
  domIsReady(function () {
    ...
    var aside = document.getElementById("aside-load"),
      close_button = document.getElementById("aside-load-closeBtn");

    if (close_button) {
      close_button.addEventListener("click", closeLoader);
    }

    var timeout, secondTimeout;
    function closeLoader() {
      if (timeout === "number") {
        window.clearTimeout(timeout);
      }
      if (secondTimeout === "number") {
        window.clearTimeout(secondTimeout);
      }

      lazyLoad();
      timeout = setTimeout(function () {
        if (aside.className.indexOf("hide") === -1) {
          aside.className += " hide";
        }
        if (close_button.className.indexOf("hide") === -1) {
          close_button.className += " hide";
        }
      }, 250);
    }
    ...
    function open_loader(loader, callBack) {
      ...
      if (loader.classesToAppend) {
        aside.className += " " + loader.classesToAppend;
      }
      if (loader.info != undefined) {
        document.getElementById("aside-load-dynamic").textContent = loader.info;
      }
      if (loader.enableCloseButton) {
        close_button.className = close_button.className.replace(" hide", "");
        var openLoaderTimeout = undefined;
        openLoaderTimeout = setTimeout(function () {
          aside.className = aside.className.replace(" hide", "");
          window.clearTimeout(openLoaderTimeout);
          callCallback();
        }, 375);
        return;
      }
    ...
    function load_map() {
      ...
      $script(google_path, function () {
        ...
        var mapCenterPosition = _isSalonNick
          ? new google.maps.LatLng(45.34773, -75.78687)
          : new google.maps.LatLng(45.35264, -75.65095);
        var mapOptions = {
            zoom: 17,
        ...
        // Set coloured marker arrow at store address.
        var _src = _isSalonNick ? "svg/loc-r.svg" : "svg/loc-p.svg",
          _path = _isEnglish
            ? _isSalonNick
              ? _pi + _src
              : "../" + _pi + _src
        ...
        var marker = new google.maps.Marker({
          ...
          map: map,
          icon: icon,
          content: _isSalonNick ? "Salon Nick" : "Salon Nick 101",
        });
        marker.setMap(map);
      });
    }
